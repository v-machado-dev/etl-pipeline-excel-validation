...
from data_validator import PlanilhaVendas
...
def validar_dados(df):
    ...
    for index, row in df.iterrows():
        try:
            # Converts DF's row to dictionary
            dados = row.to_dict()
            
            # Validate data using User model 
            ...
            if st.button("Validar Dados"):
                with st.spinner("Validando dados..."):
                    ...
                    if erros:
                        ...
                        # Amount of validated registers
                        st.write(f"Total de registros validados: {len(dados_validados)}")
                        
                        # Creates an option to download data
